export default async function handler(req, res) {
  ...
  if (!token) {
    return res.status(500).json({ ok: false, error: 'HubSpot integration is not configured yet.' });
  }

  const {
    firstName = '', lastName = '', email = '', phone = '', zip = '',
    projectType = '', paintingType = '', budget = '', start = '', details = ''
  } = req.body || {};

  if (!firstName || !lastName || !email || !phone || !zip || !projectType) {
  ...
  async function hs(path, options = {}) {
    ...
    if (!r.ok) {
      const err = new Error(data?.message || `HubSpot request failed (${r.status})`);
      err.status = r.status;
      err.data = data;
      throw err;
    }
  ...
  try {
    ...
    if (search.total > 0) {
      ...
      await hs(`/crm/v3/objects/contacts/${contactId}`, {
        method: 'PATCH',
        body: JSON.stringify({ properties: { firstname: firstName, lastname: lastName, phone, zip } })
      });
    } else {
      const contact = await hs('/crm/v3/objects/contacts', {
        method: 'POST',
        body: JSON.stringify({ properties: { firstname: firstName, lastname: lastName, email, phone, zip } })
      });
